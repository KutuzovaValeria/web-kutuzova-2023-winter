...
// Создание элемента всплывающего уведомления и добавление его в документ
    ...
    let alertPlaceholder = document.getElementById('div-alerts');
...
const alertTrigger = document.getElementById('send-modal-for-alert');
if (alertTrigger) {
    alertTrigger.addEventListener('click', () => {
        appendAlert('Nice, you triggered this alert message!', 'success');
    });
}
const api_key = "74532545-09e9-432a-9036-d38b1a85c0a6";
...
function GuideBtnHandler(event) {
    ...
    const old = document.querySelector('.modal-fast-checkbox');
    const souvenirs = document.querySelector('.modal-transfer-checkbox');
    hoursNumber.value = "";
    date.value = "";
    time.value = "";
    peopleCount.value = "";
...
function appFormModalHandler() { 
    ...
    price = {
        ...
        numberOfVisitors: Number(peopleCount.value),
        oldCoef: oldCoef,
    };
    updatePrice();
}
...
function modalBtnSendHandler() { 
    ...
    console.log('rteasdsaassadasdsadd');
...
function updatePrice() {
    ...
    const oldCheckbox = document.querySelector('.modal-fast-checkbox');
    const souvenirsCheckbox = document.querySelector('.modal-transfer-checkbox');
    const oldExtra = document.querySelector('.modal-option-fast-extra');
    const souvenirsText = document.querySelector('.modal-option-transfer-extra');
    ...
    let fPrice = Math.round(базоваяСтоимость * price.oldCoef);

    if (oldCheckbox.checked)
        oldExtra.innerHTML = `(${Math.round(базоваяСтоимость * (price.oldCoef - 1))}₽)`;
    else
        oldExtra.innerHTML = "";

    if (souvenirsCheckbox.checked) {
        
        fPrice = Math.round(fPrice + (500 * price.numberOfVisitors));
        souvenirsText.innerHTML = `(+` + String(500 * price.numberOfVisitors) + `)`;
        finalPrice.innerHTML = 'Итоговая стоимость: ' + fPrice + "₽";
    } else {
        souvenirsText.innerHTML = "";
...
function dataCorrectnessCheck() { 
    ...
    sendData.append('optionFirst', old.checked ? '1' : '0');
    sendData.append('optionSecond', souvenirs.checked ? '1' : '0');
    return flag;
}
...
function hideAlert() {
    var alertSection = document.getElementById('alert');
...
function fetchRoutes() {
    return fetch('http://exam-2023-1-api.std-900.ist.mospolytech.ru/api/routes?api_key=74532545-09e9-432a-9036-d38b1a85c0a6')
    .then(response => response.json())
    .then(data => {
        data.forEach(route => {
            routesById[route.id] = route.name;
        });
    })
...
function fetchOrders() {
    fetch('http://exam-2023-1-api.std-900.ist.mospolytech.ru/api/orders?api_key=74532545-09e9-432a-9036-d38b1a85c0a6')
    .then(response => response.json())
    .then(data => {
        ...
        data.forEach((order, index) => {
            ...
            cellRoute.textContent = routesById[order.route_id] || 'Неизвестный маршрут';
